-- 103: Fix esp_public_register() not actually being all-or-nothing.
--
-- Found during live verification: a plate-collision attempt (registering a
-- different customer with an already-registered plate) correctly returned
-- 'plate_already_registered_to_another_customer', but a real customer row
-- had still been created and committed. RETURN inside a plpgsql function
-- does NOT roll back statements already executed earlier in the same call
-- -- it only rolls back on an unhandled exception. The original version
-- created/updated the customer BEFORE re-checking plate conflicts, so a
-- rejected registration still left a real (if useless) customer row behind.
--
-- Fix: resolve the existing customer by phone as a READ-ONLY lookup first,
-- check every vehicle plate for a conflict against that resolved id (or
-- against "no existing customer yet" if none matched), and RETURN on any
-- conflict before any INSERT/UPDATE happens at all. Only once every check
-- has passed does the function start writing.
