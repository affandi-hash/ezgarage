-- 026: Fix create_tenant_user — insert auth.identities row so the email
-- provider is properly attached. Without this, the created user has no
-- identity and cannot log in, reset their password, or receive magic links.
...
BEGIN
  ...
  -- 2. Attach the email identity (this is what was missing — without this
  --    the user has no provider and cannot log in or receive reset emails)
  INSERT INTO auth.identities (
    ...
    v_new_user_id,
    lower(p_email),
    jsonb_build_object('sub', v_new_user_id::text, 'email', lower(p_email), 'email_verified', true),
    'email',
    now(),
    now(),
    now()
