...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!invoice_id || !['fpx', 'duitnow'].includes(payment_method)) {
      return new Response(JSON.stringify({ error: 'invoice_id and payment_method ("fpx" or "duitnow") are required' }), { status: 400, headers: corsHeaders })
    }
