...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Visibility into *why* something wasn't reconciled, not just that it
    // wasn't -- RaudhahPay's docs have been wrong often enough (create-bill,
    // check-bill, "paid" vs "success") that "not paid" deserves seeing the
    // actual raw status rather than assuming it means genuinely still pending.
    const notPaid: Array<{ invoice_id: string; bill_id: string; status: unknown; raw: unknown }> = []
    ...
    for (const inv of rows) {
      ...
      try {
        ...
        if (statusData.status !== 'paid') {
          notPaid.push({ invoice_id: inv.id, bill_id: inv.raudhahpay_bill_id, status: statusData.status, raw: statusData })
          continue
        }
    ...
    return new Response(JSON.stringify({ checked, reconciled, errors, notPaid }), {
